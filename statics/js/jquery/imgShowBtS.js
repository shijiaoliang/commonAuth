function ImgShow() {
    ...
    var imgTag = theEvent.srcElement;
    if (!imgTag) {
        imgTag = theEvent.target;
    }
    //var imgTag = (window.event) ? event.srcElement : event.target;
    ...
    tag.style.cssText = "width:100%;height:" + Math.max(document.body.clientHeight, document.body.offsetHeight, document.documentElement.clientHeight) + "px;position:absolute;background:black;top:0;filter: Alpha(Opacity=50);Opacity:0.5;z-index: 3;";
    tag.ondblclick = closes;//双击关闭
    var tagImg = document.createElement("div");
    tagImg.style.cssText = "font:12px /18px verdana;overflow:auto;text-align:center;position:absolute;width:200px;background:white;color:white;z-index: 4;left:" + (parseInt(document.body.offsetWidth) / 2 - 100) + "px;top:" + (document.documentElement.clientHeight / 3 + tagTop) + "px;"
    tagImg.innerHTML = "<div style='padding:10px;background:#cccccc;border:1px solid white'><img src='/statics/img/loading.gif' /><br /><br /><b style='color:#999999;font-weight:normal'>Image loading...</b><br /></div>";
    tagImg.oncontextmenu = function() {
        var clsOK = confirm("确定将关闭图片？");
        if (clsOK) {
            closes();
        }
        ;
    ...
    tagImg.ondblclick = closes;//双击关闭
    var closeTag = document.createElement("div");
    closeTag.style.cssText = "position:absolute;left:10px;top:10px;";
    //var closesHtml = "<i class='fa fa-times text-danger text' style='filter:Alpha(Opacity=100);Opacity:1;cursor:pointer;font-size:16px;z-index: 4;'></i>";
    var closesHtml = "";
   // closeTag.innerHTML = closesHtml + " 提示:双击图片缩放";
    ...
    var img = new Image();
    img.src = imgPath;
    img.style.cssText = "filter: Alpha(Opacity=0);Opacity:0;cursor:pointer;";
    var barShow, imgTime;
